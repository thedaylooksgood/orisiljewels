...
export function generateSKU(id: number): string {
  return `OJS${String(id).padStart(5, '0')}`;
}

export function getProductTags(product: Product): string[] {
  const tags = [product.category, '925 Silver', 'Sterling Silver'];
  if (product.isNewArrival) tags.push('New Arrival');
  if (product.isBestseller) tags.push('Bestseller');
  if (product.name.toLowerCase().includes('ring')) tags.push('Ring');
  if (product.name.toLowerCase().includes('earring')) tags.push('Earring');
  if (product.name.toLowerCase().includes('necklace')) tags.push('Necklace');
  if (product.name.toLowerCase().includes('pendant')) tags.push('Pendant');
  if (product.name.toLowerCase().includes('bracelet')) tags.push('Bracelet');
  if (product.name.toLowerCase().includes('toe')) tags.push('Toe Ring');
  return [...new Set(tags)];
}

export const METAL_COLORS = [
  { name: 'Silver', hex: '#C0C0C0' },
  { name: 'Rose Gold', hex: '#E0B4B8' },
  { name: 'Gold', hex: '#D4AF37' },
] as const;

export const RING_SIZES = ['5', '6', '7', '8', '9', '10'] as const;
